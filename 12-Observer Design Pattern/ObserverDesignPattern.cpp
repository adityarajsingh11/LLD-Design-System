...
int main() {
    ...
    Subscriber* subs2 = new Subscriber("Bittu", channel);

    // Aditya and Bittu subscribe to CoderArmy
    channel->subscribe(subs1);
    channel->subscribe(subs2);

    // Upload a video: both Aditya and Bittu are notified
    channel->uploadVideo("Observer Pattern Tutorial");

    // Aditya unsubscribes; Bittu remains subscribed
    channel->unsubscribe(subs1);

    // Upload another video: only Bittu is notified
