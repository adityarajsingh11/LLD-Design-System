...
// Note : I have commented out the mutex code for simplicity, but in a real multi-threaded environment, you would want to ensure thread safety when accessing shared resources like the CouponManager's linked list of coupons.
becoz code is not working with mutex, you can uncomment the mutex code and include the necessary header to make it thread-safe.